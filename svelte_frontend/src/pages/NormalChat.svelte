    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        if (content.hasOwnProperty("message")) {
          ...
          console.log(
            `Received new message: ${JSON.stringify(content.newMessage)}`
          );
...
{#if userName === ""}
  ...
  <div class="flex flex-column m2 rounded items-center">
    ...
    {:else}
      ...
      />
    {/if}
    <button id="connect" on:click={tryToConnectUser} class="rounded p1"
      >Connect</button
    >
...
{:else}
  ...
  <div class="flex flex-column">
    ...
    <div class="grid grid-cols-10">
      ...
      />
      <button id="sendmessage" class="col-span-1" on:click={sendChatMessage}
        >Send</button
      >
    </div>
  </div>
