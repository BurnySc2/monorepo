...
docker exec garage /garage layout assign -z local -c 1G "$NODE_ID"
CURRENT=$(docker exec garage /garage layout show 2>&1 | grep 'Current cluster layout version:' | grep -oP '\d+$')
