...
import discrete_structures.combination


@given(my_list=st.lists(st.builds(object)), n=st.integers())
def test_fuzz_combination_generator(my_list, n):
    discrete_structures.combination.combination_generator(my_list=my_list, n=n)
...
def test_fuzz_combinations_with_replacement_generator(iterable, replacement):
    discrete_structures.combination.combinations_with_replacement_generator(iterable=iterable, replacement=replacement)


@given(repeat=st.integers())
def test_fuzz_product_generator(repeat):
    discrete_structures.combination.product_generator(repeat=repeat)
