...
// @ts-expect-error - d3-time-format has no types
...
function render_chart() {
    ...
    series.forEach((driver_series) => {
        ...
        for (let i = 1; i < data.length; i++) {
            const prev = data[i - 1]
            const curr = data[i]
            const is_hovered = hovered_date && Math.abs(curr.date.getTime() - hovered_date.getTime()) < 86400000
        ...
        if (tooltip_data.length > 0) {
            ...
            div.innerHTML = `
                    ...
                    <div class="tooltip-entries">
                        ${tooltip_data
                            .map((point) => {
                                const point_color = series.find((s) => s.data.some((p) => p === point))?.color ?? "#ccc"
                                return `
